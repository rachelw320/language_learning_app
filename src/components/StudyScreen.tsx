...
export default function StudyScreen({ mode, category: _cat, chunkIndex: _ci, cards, isMix: _isMix, onBack, onComplete }: Props) {
	...
	const FeedbackPanel = ({ revealArabic }: { revealArabic: boolean }) => (
		<div
			className={`fade-in rounded-2xl px-5 py-4 border ${answerState === 'correct' ? 'border-success/40 bg-success/10' : 'border-danger/40 bg-danger/10'}`}
		>
			<div className="flex items-baseline gap-2 mb-2">
				<span className={`font-bold text-base ${answerState === 'correct' ? 'text-success' : 'text-danger'}`}>
					{answerState === 'correct' ? '✓ Correct!' : '✗ Not quite'}
				</span>
				{answerState === 'incorrect' && <span className="text-textSecondary text-xs">{Math.round(matchScore * 100)}% match</span>}
				{answerState === 'correct' && isMastered && <span className="text-yellow-400 text-xs font-semibold">⭐ Mastered!</span>}
			</div>
			...
			{answerState === 'correct' && isMastered && (
				<button onClick={handleDismiss} className="mt-3 text-xs text-textSecondary underline pressable">
		...
		return (
			<div className="flex flex-col h-full safe-top">
				...
				<div className="flex-1 flex flex-col px-5 py-5 gap-4 overflow-y-auto">
					<div
						className="bg-surface border border-border rounded-3xl px-6 py-6 flex items-center justify-between gap-4"
					...
					<button onClick={handleSkip} className="text-textSecondary text-sm text-center pressable py-2">
						Skip
					</button>
				</div>
			</div>
